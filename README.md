...
プログラムが「期待通りに正しく動くか」を自動でチェックすることを**ユニットテスト**と呼びます。本プロジェクトでは、以下の2つのテストを行い、シミュレーションの信頼性を保証します。

### 5.1 ユニットテストとは？
ユニットテストは、プログラム全体を動かす前に、特定の小さな機能（ユニット）が正しく動くかを確認する「小テスト」のようなものです。
- **どうやるのか**: 「この入力を与えたら、この出力が返ってくるはずだ」という期待値をあらかじめ用意しておき、プログラムの実行結果と一致するかを自動で判定させます。

### 5.2 最小系 ($L=2$) での答え合わせ
シミュレーションが「物理的に正しい答え」を出しているかを確かめる最も強力な方法です。

- **なぜ $L=2$ なのか？**:
  $L=2$ の場合、サイト数は $2 \times 2 = 4$ 個です。スピンの状態は各サイトで 2 通りなので、系全体の状態は $2^4 = 16$ 通りしかありません。
- **どうやるのか**:
  1. **手計算（厳密解）**: 16通りの全状態についてエネルギーを計算し、統計力学の公式（分配関数）を使って、平均エネルギーや磁化の「理論上の値」を正確に算出します。
  2. **シミュレーション**: $L=2$ で実際にプログラムを走らせます。
  3. **比較**: 両者が（統計的な誤差の範囲内で）一致すれば、アルゴリズムが正しいことが証明されます。

### 5.3 計算スピードの実測 ($O(L^2)$ の検証)
プログラムが「効率よく動いているか」を確かめるテストです。

- **何を確かめるのか**:
  1 MCS（1ステップ）の計算時間は面積 $L^2$ に比例するはずです。
- **どうやるのか**:
  1. サイズ $L=8, 16, 32, 64$ と順番に大きくしながら、1 MCS にかかった時間を計測します。
  2. 例えば $L$ を 2 倍（$8 \to 16$）にしたとき、計算時間が **4 倍** になっていれば合格です。
  3. もし時間がそれ以上（例：10倍など）かかっているなら、どこかに無駄な計算が紛れ込んでいることが分かります。
