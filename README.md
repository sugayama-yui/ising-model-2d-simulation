...
- **なぜメトロポリス法か？**: イジング模型の状態数は $2^{L^2}$ 通りあり、$L=32$ でも約 $10^{308}$ 通りという天文学的な数になります。これをすべて計算するのは不可能なため、重要サンプリング（確率的に重要な状態だけを拾い出す）を行うメトロポリス法が最適です。
- **なぜビンダー累積量か？**: 磁化や比熱のピークを見るだけでは、サイズ $L$ による「ズレ」が生じ、無限系の真の $T_c$ を特定できません。ビンダー累積量は「サイズに依存しない交点」を持つため、最も精度よく $T_c$ を特定できます。
